...
export const WorkSection = (projects) => {
  const jobs = projects.projects.map((project) => {
    ...
    return (
      <CustomCard textSide={textSide} project={project} uniqKey={uniqid()} />
    );
  });

  return (
    <Container className="col-10 col-sm-8 col-md-6 d-flex flex-column section" id="work-section" key={uniqid()}>
